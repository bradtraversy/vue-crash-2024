...
import { JobSchema, type Company, type Job } from '@/services/jobs';
...
const state = reactive<{ job: Job | null }>({ job: {} as Job });
const handleSubmit = async () => {
  ...
  const updatedJob = JobSchema.parse(jobObj);
  const response = await jobsService.editJob(updatedJob);
  router.push(`/jobs/${response.id}`);
};
onMounted(async () => {
  const data = await jobsService.getJob(jobId.toString());
  if (data) {
    state.job = data;
